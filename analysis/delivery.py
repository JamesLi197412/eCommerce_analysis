...
def delivery_performance(df):
    ...
    df['carrier_response'] = np.ceil(
        (df['order_delivered_carrier_date'] - df['order_approved_at']) / pd.Timedelta(hours=1))

    # Visualisation for these results
    # Normal distribution of delta and gap
    hist_plot(df, 'purchase_approved_delta', 'Count', 'purchase_approved_delta',
              path=f'output/visualisations/delivery/purchase_approved_delta.png')

    hist_plot(df, 'carrier_response', 'Count', 'carrier_response',
              path=f'output/visualisations/delivery/carrier_response.png')

    # Delivery Status
    pie_chart(df, 'delivery_status', 'order_id', sns.color_palette('Set2'), 'Delivery Status',
              path=f'output/visualisations/delivery/delivery status.png')
...
def delivery_prediction(df):
    ...
    delivered['cities distances'] = (
        np.sqrt(((delivered["geolocation_lat_cust"] - delivered["geolocation_lat_seller"]) ** 2) +
                ((delivered["geolocation_lng_cust"] - delivered["geolocation_lng_seller"]) ** 2)))

    delivered['product volumn'] = delivered['product_length_cm'] * delivered['product_height_cm'] * delivered[
        'product_width_cm']

    # Extract features
    features = ['cities distances', 'cities differences', 'delivery_status', 'purchase_approved_delta', 'customer_city',
                'seller_city', 'product volumn', 'product_weight_g']
    target = ['carrier_response']

    encode_features = ['customer_city', 'seller_city']
    numerical_features = ['cities distances', 'cities differences', 'delivery_status', 'purchase_approved_delta',
                          'product volumn', 'product_weight_g']
