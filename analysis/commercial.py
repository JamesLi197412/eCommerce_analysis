...
def order_customer(orders, customers, payment, items, products, product_category):
    ...
    popular_category(orders_customers_items, 'product_category_name_english', 'order_id',
                     'output/visualisations/commercial/Treemap of product category.png')
...
def customer_split_analysis(customer_frequency, orders_customers_items):
    ...
    popular_category(df, 'customer_state', 'customer_unique_id',
                     'output/visualisations/commercial/customer buy once state distribution.png')

    popular_category(df, 'product_category_name_english', 'customer_unique_id',
                     'output/visualisations/commercial/customer buy once product distribution.png')
...
def product_popularity(df, path):
    product_df = df.groupby(['product_category_name'])['customer_unique_id'].agg(['count']).reset_index().sort_values(
...
def yearly_new_client(df):
    ...
    client_first = df.groupby(['customer_unique_id'])['order_purchase_timestamp'].agg(['min']).reset_index().rename(
        columns={'min': 'first_purchase_date'})
    ...
    yearly_clients = client_first.groupby(['order_year', 'order_month'])['customer_unique_id'].agg(
        ['count']).reset_index().rename(columns={'count': 'new clients'})
...
def geolocation_sales(orders_customers_payment, geolocation):
    ...
    month_state_sales = orders_customers_payment.groupby(['order_month', geolocation])['payment_value'].agg(
        ['count', 'sum']).reset_index()
